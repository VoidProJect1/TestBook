...
# =============================================================================
# WEB SEARCH
# Priority 1: SearXNG (self-hosted, unlimited) — only if SEARXNG_URL is set
# Priority 2: DDG Lite (direct HTTP POST)
# Priority 3: duckduckgo-search library
# =============================================================================
...
def _ddg_lite(query: str, max_results: int) -> List[Dict]:
    ...
    try:
        ...
        # DDG Lite uses <table> rows — find all result links
        for a in soup.find_all("a", class_="result-link"):
            if len(results) >= max_results:
                break
            title   = a.get_text(" ", strip=True)
            url     = a.get("href", "")
            # snippet is in a sibling/nearby td with class result-snippet
            snippet = ""
            parent  = a.find_parent("tr")
            if parent:
                next_tr = parent.find_next_sibling("tr")
                if next_tr:
                    snip = next_tr.find("td", class_="result-snippet")
                    if snip:
                        snippet = re.sub(r"\s+", " ", snip.get_text(" ", strip=True)).strip()
            if title and url:
...
def web_search(query: str, max_results: int = 4) -> List[Dict]:
    ...
    if SEARXNG_URL:
        ...
        if results:
            ...
            return results
        print("[Search] SearXNG failed → DDG Lite")

    # 2. DDG Lite
    results = _ddg_lite(query, max_results)
    if results:
        print(f"[Search] DDG Lite returned {len(results)} results")
        _cache_set(cache_key, results)
        return results
    print("[Search] DDG Lite failed → library")

    # 3. Library
    results = _ddg_library(query, max_results)
    if results:
        print(f"[Search] DDG Library returned {len(results)} results")
        _cache_set(cache_key, results)
    else:
        print("[Search] All search methods failed")
    return results
    ...
    for key in keys:
        ...
        try:
            r = http.post(
                ...
                json={"model": "grok-2-latest", "messages": msgs, "max_tokens": 1024},
                timeout=30,
            )
            if r.status_code == 403: _dead_keys.add(tag); continue
            if r.status_code == 429: continue
            if r.status_code != 200:
                print(f"[Grok] HTTP {r.status_code}: {r.text[:200]}")
                continue
...
def handle_question(chat_id: int, uid: int, text: str, force_web: bool = False):
    ...
    try:
        ...
        if voice_on:
            ...
            edit_msg(chat_id, msg_id, safe_send_text(answer))
