...
def handle_image(chat_id: int, uid: int, photo_sizes: list, caption: str = ""):
    if not check_limit(uid, "images"):
        send_msg(chat_id, "⚠️ Image limit reached for this hour.")
        return
    ...
    try:
        ...
        prompt    = caption or "Describe this image in detail. Cover objects, colors, mood, composition, and any visible text."
        extra_ctx = None

        if get_model(uid) == "pro":
            pub_url = upload_telegraph(img_bytes)
            results = web_search(pub_url if pub_url else prompt, max_results=3)
            if results:
...
def _run_web_more(chat_id: int, uid: int, msg_id: int, page: int, query: str):
    """Fetch more search results for a given page."""
    results = web_search(query, max_results=5 * page)
    # Slice to show only this page's batch
    page_results = results[(page - 1) * 5 : page * 5]
    if not page_results:
        tg("editMessageText", {
            "chat_id":    chat_id,
            "message_id": msg_id,
    ...
    html   = _format_web_results_html(query, page_results, page=page)
    markup = _web_results_markup(query, page=page, has_more=len(page_results) >= 5)
    tg("editMessageText", {
        "chat_id":                  chat_id,
        "message_id":               msg_id,
        "text":                     html[:4000],
...
def handle_callback(cb_id: str, uid: int, chat_id: int, msg_id: int, data: str):
    ...
    # ── Web search: redo search ─────────────────────────────────────
        ...
        threading.Thread(
            target=_run_web_more,
            args=(chat_id, uid, msg_id, 1, query),
            daemon=True,
        ).start()
