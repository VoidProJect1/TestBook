...
def handle_limit(chat_id: int, uid: int, args: list, reply_uid: int = None):
    ...
    # Reply-based: /limit 2  while replying to a user  → multiply that user's limit by 2
    if reply_uid and len(args) == 1:
        try:
            mult = float(args[0])
        except ValueError:
            send_msg(chat_id, "Usage: reply to a user and send /limit MULTIPLIER"); return
        ...
        save_cfg(cfg)
        send_msg(chat_id, f"User {reply_uid} limit: {current}x × {mult} = <b>{new_mult}x</b>")
        return
    ...
    if len(args) == 2:
        ...
        send_msg(chat_id,
            "Usage:\n"
            "/limit MULTIPLIER — set global limit multiplier\n"
            "/limit USER_ID MULTIPLIER — set per-user multiplier\n"
            "↩️ Reply to a user + /limit N — multiply that user's limit by N\n\n"
            "<b>Set per-tier limits (admin):</b>\n"
            "/setlimit TIER ACTION VALUE\n"
            "  e.g. /setlimit pro images 20\n"
...
def handle_callback(cb_id: str, uid: int, chat_id: int, msg_id: int, data: str):
    ...
    # ── Admin panel ─────────────────────────────────────────────────
        ...
        if action == "stats":
            ...
            lines  = ["⚡ <b>Rate Limits</b> <i>(per hour)</i>\n"]
            ...
            lines.append(
                ...
                "<b>Multipliers:</b>\n"
                "<code>/limit MULTIPLIER</code> — global\n"
                "<code>/limit USER_ID MULTIPLIER</code> — per user\n"
                "↩️ Reply to user + <code>/limit N</code>"
            )
            edit_msg(chat_id, msg_id, "\n".join(lines))
