...
# =============================================================================
# TYPEWRITER EFFECT  — edits message progressively to give "AI is typing" feel
# Stages: short chunk → medium chunk → full answer
# =============================================================================
...
def _get_chunks(text: str) -> List[str]:
    """
    Split full answer into 3 progressive chunks for typewriter feel.
    Stage 1: ~first 10 words  (feels like AI started)
    Stage 2: ~first 40 words  (feels like AI is typing more)
    Stage 3: full text        (complete answer)
    """
    words = text.split()
    total = len(words)

    if total <= 15:
        # Short answer — just show it in 2 steps
        mid = max(1, total // 2)
        return [
            " ".join(words[:mid]),
            text,
    ...
    # For longer answers, 3 stages
    s1 = max(8,  total // 6)          # ~first 10-15 words
    s2 = max(25, total * 2 // 3)      # ~first 60% of words
    return [
        " ".join(words[:s1]) + " …",
        " ".join(words[:s2]) + " …",
        text,
...
def typewriter_edit(chat_id: int, msg_id: int, full_text: str, stop: threading.Event):
    """
    Edits the message progressively in chunks with small delays.
    Gives user the feeling the AI is typing in real-time.
    """
    ...
    # Delays between edits (seconds) — feels natural
    delays = [0.6, 0.9, 0.0]

    for i, chunk in enumerate(chunks):
        if i == len(chunks) - 1:
            # Final edit — use full formatted text
            display = converted_full
        else:
            display = safe_send_text(chunk)
        ...
        try:
            ...
            print(f"[Typewriter] edit {i+1} failed: {e}")

        # Wait before next chunk (skip delay on last)
        if i < len(chunks) - 1:
...
def web_search(query: str, max_results: int = 5) -> List[Dict]:
    ...
    if cached:
        print(f"[Search] cache hit for: {query[:40]}")
        return cached
...
# Grok: VERIFIED model alias strings (from docs.x.ai/docs/models April 2026)
# grok-3-fast-beta and grok-3-beta are the confirmed API aliases
GROK_MODELS  = [
    "grok-3-fast-beta",      # fastest flagship (alias: grok-3-fast-latest)
    "grok-3-beta",           # full flagship   (alias: grok-3-latest)
    "grok-3-mini-fast-beta", # lightweight reasoning fallback
]
...
def _try_grok(prompt: str, system: str, history: list) -> Optional[str]:
    ...
    if not keys:
        print("[Grok] no keys configured")
        return None

    msgs = _openai_messages(system, history, prompt)
    for model in GROK_MODELS:
        for key in keys:
            tag = f"grok:{key[:8]}:{model}"
            if tag in _dead_keys: continue
            try:
                r = http.post(
                    GROK_BASE,
                ...
                if r.status_code in (401, 403):
                    print(f"[Grok] key dead ({r.status_code}): {r.text[:100]}")
                    _dead_keys.add(tag)
                    continue
                if r.status_code == 429:
                    print(f"[Grok] {model} rate limited")
                    continue
                if r.status_code == 404:
                    # model doesn't exist, try next
                    print(f"[Grok] model {model} not found")
                    break
                if r.status_code != 200:
                    print(f"[Grok] {model} HTTP {r.status_code}: {r.text[:200]}")
                    continue

                data = r.json()
                choices = data.get("choices", [])
                if not choices:
                    print(f"[Grok] {model} empty choices")
                    continue
                text = choices[0].get("message", {}).get("content", "").strip()
                if text:
                    print(f"[Grok] success with {model}")
                    return text
                print(f"[Grok] {model} empty content")

            except requests.exceptions.Timeout:
                print(f"[Grok] {tag} timeout")
            except Exception as e:
                print(f"[Grok] {tag}: {e}")
    return None
...
def handle_help(chat_id: int):
    send_msg(chat_id,
        ...
        "/help — This menu\n\n"
        "<b>Models:</b>\n"
        "Void Lite — Fast answers, no web\n"
        "Void Flash — Balanced (default)\n"
        "Void Pro — Auto web search every message\n\n"
        "<b>Send a photo</b> for AI image analysis.\n"
        "<b>Voice replies</b> available in 10 Indian languages."
    )
...
def handle_web_cmd(chat_id: int, uid: int, query: str):
    ...
    if not check_limit(uid, "searches"):
        send_msg(chat_id, "⚠️ Search limit reached for this hour.")
        return
...
def handle_question(chat_id: int, uid: int, text: str, force_web: bool = False):
    if not check_limit(uid, "messages"):
        send_msg(chat_id, "⚠️ Message limit reached for this hour.")
        return
...
def handle_image(chat_id: int, uid: int, photo_sizes: list, caption: str = ""):
    ...
    try:
        ...
        if get_model(uid) == "pro":
            ...
            if pub_url:
                results = web_search(pub_url, max_results=3)
            else:
                results = web_search(prompt, max_results=3) if len(prompt) > 5 else []
            if results:
...
def handle_callback(cb_id: str, uid: int, chat_id: int, msg_id: int, data: str):
    ...
    if data.startswith("switch:"):
        ...
        if code == "toggle":
            ...
            edit_msg_markup(chat_id, msg_id,
                f"Voice: <b>{esc(display)}</b> selected.\nVoice replies are now ON.",
                voice_markup(uid))
...
def main():
    ...
    print(f"Grok keys:   {len(cfg.get('grok_keys',[]))}")
